...
void draw() {
  //se premi il mouse parametri di riferimento x e y della posizione del mouse
  if (mousePressed) {
    int x = mouseX;
    int y = mouseY;
...
//cancellare tutto dallo schermo o salvare immagine
void keyReleased() {
  if (key == DELETE || key == BACKSPACE) background(255);
  if (key == 's' || key == 'S') saveFrame(timestamp()+"_##.png");

  // per cambiare direzione dell'angolo
  if (key=='d' || key=='D') {
    angle = angle + 180;
